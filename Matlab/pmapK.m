...
function [rate_map,spatial_scale,SpksShuffle,oc_map,rate_mapB,FrIndex]=pmapK(Spks,Traj, msT,fstart,varargin)
verbose=0;
spON=0;
corrFlag=0;
year=2018;
  ...
  switch lower(varargin{i})
    case 'animal',
      ...
      elseif strcmp(varargin{i+1},'rat')
          ...
          Bin=BinWidthCm/cmPerPixel;%2.5cmあたりのpixel数
          kHz=25;
          spON=1;
          ThS=2.5;        
          msT=msT/kHz;
...
if isempty(Spks)
    ...
    FrIndex=[];
    if shuffle
        rate_map=zeros(shuffleN,size(dummy,2));
    end
    return;
end
